                ...
                if session_created:
                    ...
                    for attempt in range(3):
                        ...
                        print(f" ✅GraphQL Response Status: {graphql_response.status_code}")
                                                                        ...
                                                                        # ── START AUTOMATIC TAX & TERMS BYPASS LOOP ──
                        try:
                            _raw_text_resp = graphql_response.text.upper()
                            if "WAITING_PENDING_TERMS" in _raw_text_resp or "TAX_NEW_TAX_MUST_BE_ACCEPTED" in _raw_text_resp or "PENDINGTERMVIOLATION" in _raw_text_resp:
                                print(" [shopifyapi] ⚡ Vergi/Şartlar engeli yakalandı! Otomatik onaylanıp 2. kesin submit basılıyor...")
                                
                                # Sitenin güncel checkout sayfasından yeni tokenları çekiyoruz
                                ...
                                if ref.status_code == 200:
                                    ...
                                    if nst: session_token = nst.group(1)
                                ...
                                # 🎯 HAZIR KANALI OVERRIDE ET: Mevcut paketteki kargo satırını bul ve onay flagini kapat
                                try:
                                    if "delivery" in new_payload["variables"]["input"] and "deliveryLines" in new_payload["variables"]["input"]["delivery"]:
                                        for line in new_payload["variables"]["input"]["delivery"]["deliveryLines"]:
                                            if "destinationChanged" in line:
                                                line["destinationChanged"] = False
                                except Exception:
                                    pass
                                
                                # Shopify replay attack korumasını aşmak için benzersiz yeni bir istek tokenı üretiyoruz
                                new_payload["variables"]["attemptToken"] = f"{token}-final-override-{random.random()}"
                                
                                # Başlıkları yeni session token ile tazeleyelim
                                new_headers = copy.deepcopy(graphql_headers)
                                new_headers["x-checkout-one-session-token"] = session_token
                                
                                await asyncio.sleep(1.2)
                                # Esas vurucu gücü şimdi fırlatıyoruz ve orijinal değişkeni güncelliyoruz!
                                graphql_response = await session.post(graphql_url, headers=new_headers, json=new_payload)
                        except Exception as e:
                            print(f" [shopifyapi] ⚠️ Bypass loop hatası: {str(e)}")
                        # ── END AUTOMATIC TAX & TERMS BYPASS LOOP ──
